...
class _HomePageState extends State<HomePage> {
  ...
  // Управление избранными элементами через API
  void _toggleFavorite(BuildContext context, int index) async {
    final userId = 1; // Замените на реальный ID пользователя
    final product = productItems[index];
    final apiService = ApiService();

    try {
      if (await apiService.getFavoriteProducts(userId).then((favorites) => favorites.contains(product))) {
        await apiService.deleteProductFromFavorite(userId, product.id);
      } else {
        await apiService.addProductToFavorite(userId, product);
      }
  ...
  void _toggleCart(BuildContext context, int index) async {
    final userId = 1; // Замените на реальный ID пользователя
    final product = productItems[index];
    final apiService = ApiService();

    try {
      if (await apiService.getCartProducts(userId).then((cart) => cart.any((item) => item.id == product.id))) {
        await apiService.deleteProductFromCart(userId, product.id);
      } else {
        await apiService.addProductToCart(userId, product);
      }
                  ...
                  Expanded(
                    child: GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return _buildMangaCard(context, productItem, index, screenWidth);
                      },
                    ),
                  ),
                ],
  ...
  // Карточка манги
  Widget _buildMangaCard(BuildContext context, MangaItem productItem, int index, double screenWidth) {
    final isMobile = screenWidth < 600;
